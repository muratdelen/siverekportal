                    ...
                    if (in_array(YT_INSERT, $sayfaIslemleriId)) {
                        echo '<div class="col-md-8" style="	margin-bottom: 10px;">
                            <form class="form-horizontal" method="GET" action="?add">
                                <input class="btn bg-green" type="submit" id="btn_yeni_kayit_ekle" name="add" value="YENİ ASANSÖR EKLE">
                            </form>  
                        </div>';
                    }
                            ...
                            <div class="form-group form-group-sm">
                                ...
                                <div class="col-sm-8">
                                    <select class="form-control select2 select2-hidden-accessible"  onchange="$('#get-items').click();" style="width: 100%;" tabindex="-1" aria-hidden="true" id="ruhsat" name="ruhsat">
                                        ...
                                        <!--<option value='ruhsatyok'>Ruhsat No Olmayanlar</option>-->
                                        <?php
                                        try {
                ...
                if (isset($_GET['Sorgula'])) {
                    if (isset($_GET['ruhsat'])) {
                        if ($_GET['ruhsat'] == "ruhsatyok") {
                            $ItemsSQL = "SELECT
                            s_asansor_uygulama_projeleri.id, 
                                        s_ruhsat_bilgileri.ruhsat_no, 
                            s_asansor_uygulama_projeleri.blok_no,
                                       ...
                                       (CASE s_ruhsat_bilgileri.kacak_islem_yapildi_mi
                                        ...
                                        s_asansor_uygulama_projeleri.ada_parsel, 
                                        s_ruhsat_bilgileri.yibf_no,
                                        s_asansor_uygulama_projeleri.adi_soyadi, 
                                ...
                                RIGHT JOIN s_asansor_uygulama_projeleri ON s_ruhsat_bilgileri.id = s_asansor_uygulama_projeleri.s_ruhsat_bilgileri_id
                                WHERE s_ruhsat_bilgileri.aktif_mi AND s_ruhsat_bilgileri.id = ? ";
                            try {
                                $listItems = $GLOBALS['db']->fetchAll($ItemsSQL, 1);
                            } catch (Zend_Db_Exception $ex) {
                                log::DB_hata_kaydi_ekle(__FILE__, $ex);
                            }
                        } else {
                        ...
                        if (trim($_GET['kacak_islem_yapildi_mi']) !== "") {
                            $ruhsat_where_string .= " AND s_ruhsat_bilgileri.kacak_islem_yapildi_mi = ? ";
                        ...
                        $ItemsSQL = "SELECT
                            ...
                            s_asansor_uygulama_projeleri.blok_no,
                                ...
                                FROM s_ruhsat_bilgileri
                                RIGHT JOIN s_asansor_uygulama_projeleri ON s_ruhsat_bilgileri.id = s_asansor_uygulama_projeleri.s_ruhsat_bilgileri_id
                                WHERE s_ruhsat_bilgileri.aktif_mi " . $ruhsat_where_string . " LIMIT 1000";
                        try {
...
//                        var_dump($listItems, $ItemsSQL);
                    ...
                    if ((isset($listItems)) && !empty($listItems)) {
                        ...
                        $options = array(
                            ...
                            'tableHeaders' => array('RUHSAT NO', 'Blok No', 'Ruhsat Tarihi', 'Kaçak', 'Bülten No', 'Ada/Parsel', 'YİBF No', 'Adı Soyadı', 'Ruhsat Cinsi', 'Ruhsat Veriliş Amacı', 'Fenni Mesul/YDK', 'Mahallesi', 'Yapı Alanı', 'İskan', 'İskan Tarihi', 'İskan No', 'İskan Bülten No', 'Kaçak Bilgisi'),
                            //zorunlu olmayan parametreler
                            //        'id' => 'example2' , // optional
                            'order' => array(0, 'asc'),
                            'tableFooters' => array('RUHSAT NO', 'Blok No', 'Ruhsat Tarihi', 'Kaçak', 'Bülten No', 'Ada/Parsel', 'YİBF No', 'Adı Soyadı', 'Ruhsat Cinsi', 'Ruhsat Veriliş Amacı', 'Fenni Mesul/YDK', 'Mahallesi', 'Yapı Alanı', 'İskan', 'İskan Tarihi', 'İskan No', 'İskan Bülten No', 'Kaçak Bilgisi'), // optional
