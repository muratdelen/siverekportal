...
//                    echo 'Tablo verisi boş olamaz.';
        ...
        $dataTable = '<table id="' . $this->tableId . '" class="table-responsive table-bordered table-striped table-hover  dataTable" style="width:100%">';
...
//                  $('#$this->tableId  tbody > tr').eq(row_index).addClass('selected');
    ...
    static function process_table_content($aColumns, $sIndexColumn, $sTable, $sWhere, $sLinkColumn = "", $columnsName = "") {
        ...
        $_SESSION["filtersWhereSql"] = $sWhere;
        /* Individual column filtering */
        for ($i = 0; $i < count($columnsName); $i++) {
            if ($_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
                if ($sWhere == "") {
                    $_SESSION["filtersWhereSql"] = "WHERE ";
                    $sWhere = "WHERE ";
                } else {
                    $_SESSION["filtersWhereSql"] .= " AND ";
                    $sWhere .= " AND ";
                }
                ...
                if ($columnsName[$i] == "zaman") {
                    $_SESSION["filtersWhereSql"] .= $columnsName[$i] . " LIKE '%" . convertDateFormatBasicDefault($_GET['sSearch_' . $i]) . "%' ";
                } else {
                    $_SESSION["filtersWhereSql"] .= $columnsName[$i] . " LIKE '%" . $_GET['sSearch_' . $i] . "%' ";
                }
...
//            var_dump($sWhere);die();
        ...
        $sQuery = "
		SELECT SQL_CALC_FOUND_ROWS  " . $sIndexColumn . " , $sLinkColumn, " . str_replace(" , ", " ", implode(", ", $aColumns)) . "
		FROM   $sTable
		$sWhere
		$sOrder
...
//        var_dump($sQuery);
        ...
        foreach ($aRow as $value) {
            $value->yeni_mi = $value->yeni_mi ? '<div class="btn-success">Yeni</div>' : '<div class="btn-info">Okundu</div>';
            if ($GLOBALS['girisYapanKullaniciGrupId'] === 1 || $GLOBALS['girisYapanKullaniciGrupId'] === 2 || $GLOBALS['girisYapanKullaniciGrupId'] === 3 || $GLOBALS['girisYapanKullaniciGrupId'] === 4) {
                $value->aciklama = mdecrypt($value->aciklama, $value->kullanici_adi);
            }else{                
                $value->aciklama = mdecrypt($value->aciklama, $GLOBALS['kullaniciAdi']);
            }
        }
...
//            }
            if ($sLinkColumn != "") {
                $button = '
                    <div class="btn-group">
                      <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false">' . _("İşlem") . '<span class="fa fa-caret-down"></span></button>
                        <ul class="dropdown-menu">
                        <li><a href = "' . $aVal->$sLinkColumn . '" class="btn bg-purple btn-block" >' . _("Mesaj Yolla") . '</a></li>
                        <li><a href = "?remove=' . $aVal->id . '" class="btn bg-danger btn-block" ><input type="hidden" />' . _("Mesaj Sil") . '</a></li>
                      </ul>
                    </div>';
            } else {
                $button = '
                    ...
                    switch ($key) {
                        ...
                        default :
                            $button = '<li>
                             <form method="post" action="' . (isset($this->buttonUrls[$key]) ? $this->buttonUrls[$key] : null) . '">
                              <input type="hidden"  name = "' . $key . '" value="' . $id . '"/>
                                <button id = "' . $key . '" class="btn bg-red" type="submit">' . $value . '</button>
                            </form>
                        </li>';
                            break;
                    }
