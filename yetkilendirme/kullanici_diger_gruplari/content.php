...
<div class="content-wrapper">    
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"><?php echo __("Kullanıcı Diğer Grupları İşlemleri") ?></li>
        </ol>
    </section>
            ...
            if (isset($_GET['new'])) {
                ...
                if (in_array(YT_INSERT, $sayfaIslemleriId)) {
                    echo '<div class="col-md-8" style="	margin-bottom: 10px;">
                                <a href="?new" class="btn bg-purple" >KULLANICI DİĞER GRUP EKLE</a>  
                        </div>';
                }
                if (in_array(YT_QUERY, $sayfaIslemleriId)) {//eğer sorgula butonuna basıldı ise sor
                    ...
                    if (isset($_GET['kullanici_adi'])) {
                        $ItemsSQL = "SELECT
                                            yt_kullanici_diger_gruplari.id,
                        ...
                        try {
                            $listItems = $GLOBALS['db']->fetchAll($ItemsSQL, array($_GET['kullanici_adi'], "%" . $_GET['kisi_adi_soyadi'] . "%", "%" . $_GET['kisi_adi_soyadi'] . "%"));
                        } catch (Zend_Db_Exception $ex) {
                            log::DB_hata_kaydi_ekle(__FILE__, $ex);
                        }
                        ...
                        if ((isset($listItems)) && !empty($listItems)) {
                            ...
                            $options2 = array(
                                ...
                                'tableHeaders' => array('Kullanıcı Adı', 'Adı', 'Soyadı', 'Ünvanı', 'Grubu', 'Aktif Mi?'),
                                //zorunlu olmayan parametreler
                                //        'id' => 'example2' , // optional
                                'order' => array(0, 'asc'),
                                'tableFooters' => array('Kullanıcı Adı', 'Adı', 'Soyadı', 'Ünvanı', 'Grubu', 'Aktif Mi?'),
